...
-- 3. Give the id and the name for the stops on the '4' 'LRT' service.
  SELECT id, name
  FROM stops AS st
  JOIN route AS ro ON st.id = ro.stop
...
-- Run the query and notice the two services that link these stops have a count of 2. Add a HAVING clause to restrict the output to these two routes.
  ...
  HAVING company = 'LRT' AND COUNT(*) = 2;
...
-- by joining two copies of the stops table we can refer to stops by name rather than by number. Change the query so that the services between 'Craiglockhart' and 'London Road' are shown.
